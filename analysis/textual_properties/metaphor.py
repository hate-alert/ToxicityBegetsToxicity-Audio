...
df = pd.read_csv('/kaggle/input/lcc-balanced/LCC_Label_Balanced.csv')
...
def metaphor(text):
    ...
    return pred_bools[0][1].cpu().numpy()
...
for chain in tqdm(data["chains"]):
    ...
    for element in chain["Before Conversations"]:
        arr = metaphor(element["text"])
        element["Metaphor"] = bool(arr)
        if prev_text == "":
            prev_text = element["text"]
            continue
        joined_text = prev_text + " " + element["text"]
        arr = metaphor(joined_text)
        element["Concatenated_metaphor"] = bool(arr)
        prev_text = element["text"]
